...
------------------------
Runs feature-level hard ablations (alpha = 0) on top CMA mediators.

Helper functions from sfc_utils are inlined for readability. Switch SAE type by
changing the SAE_MODE constant below.
"""
...
# =========================================================================
# Helper structures (copied from sfc_utils)
# =========================================================================
...
def run_local_cut(
    ...
    for idx, (mediator, category) in enumerate(tqdm(entries, desc="Mediators")):
        ...
        bias_base_orig, bias_base_gated, ppl_orig, ppl_orig_gated, nie_orig = apply_alpha_gate(
            model,
            submodule,
            dictionary,
            base_prompt,
        ...
        for feat_idx, feat_score in zip(top_indices, top_scores):
            ...
            if isinstance(feat_score, (list, tuple)):
                if not feat_score:
                    ...
                    feat_score = feat_score[0]
            _, bias_cut_after, _, ppl_cut_after, nie_cut = apply_alpha_gate(
                model,
                submodule,
                dictionary,
                base_prompt,
            ...
            results.append({
                "analysis": "local_cut",
                "edit_label": "feature_cut",
                "feature_idx": int(feat_idx),
                "feature_score": float(feat_score),
                "alpha": 0.0,
                "nie_original": float(nie_orig),
                "nie_ablated": float(nie_cut),
                "delta_nie": float(nie_cut - nie_orig),
                "ppl_original": float(ppl_orig),
                "ppl_ablated": float(ppl_cut_after),
                "delta_ppl": float(ppl_cut_after - ppl_orig),
                ...
                "bias_edited": float(bias_cut_after),
            })
...
def plot_local_cut_results(rows: List[Dict], csv_path: str) -> None:
    ...
    categories = [r.get("mediator_category", "topk") for r in rows]
    color_map = {"topk": "tab:blue", "control": "tab:orange"}

    plt.figure(figsize=(8, 5))
    for x, y, cat in zip(abs_nie, delta_ppl, categories):
        plt.scatter(x, y, c=color_map.get(cat, "gray"), alpha=0.6, label=cat)
    handles = []
    seen = set()
    for cat, color in color_map.items():
        handles.append(plt.Line2D([0], [0], marker="o", color="w", label=cat, markerfacecolor=color, markersize=8))
        seen.add(cat)
    plt.legend(handles, list(color_map.keys()))
    plt.xlabel("|ΔNIE|")
    plt.ylabel("ΔPPL")
    plt.title("Local Cut (Feature Zeroing)")
